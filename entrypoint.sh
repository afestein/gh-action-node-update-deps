...
npx npm-check-updates -u --packageFile ${WORKING_DIRECTORY}/package.json --target ${TARGET}
...
if [ -n "${BUMP_VERSION}" ]; then
  if [ "${PACKAGE_MANAGER}" == 'npm' ]; then
    npm version --no-git-tag-version ${BUMP_VERSION} --prefix ${WORKING_DIRECTORY}
  elif [ "${PACKAGE_MANAGER}" == 'yarn' ]; then
    yarn version --no-git-tag-version "--${BUMP_VERSION}" --cwd ${WORKING_DIRECTORY}
  fi
fi
...
gh pr create --title "${DESCRIPTION}" -b "Generated by [${RUN_LABEL}](${RUN_ENDPOINT})." -l "${PULL_REQUEST_LABELS}"
